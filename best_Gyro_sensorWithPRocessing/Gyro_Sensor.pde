...
void inclination(int heightInlcint) {
  ...
  pushMatrix();
  translate(width-width/2, height-height/2);
  rotateY = map(heightInlcint, -125, 125, -80, 80);
  signRotateY= -(rotateY);
  //rotate(radians(-90));
  fill(#D25A0F);
  noStroke();
...
void heightMeter(int AngleY) {
  ...
  depthAngle = map(AngleY, -125, 125, (height-height*0.685), -(height-height*0.685));
  //depthAngle = map(mouseY,0,height,-(height-height*0.685),(height-height*0.685));                        //mapping the value
  rectMode(CORNER);
  fill(#E477FA);
  noStroke();
...
void arrowMeter(int angleArrow) {
  ...
  float horizonAngle = map(angleArrow, -125, 125, (height-height*0.685), -(height-height*0.685));    //angle for slider 
  int degree = int(map(angleArrow,-125,125,-90,90));                                                 //angle for box angle data
  //float meterHeight = map(horizonAngle, -100, 100,);

  //code for arrowMeter 
  beginShape();
  noStroke();
  fill(255, 0, 0);
  vertex(width-width*0.87, -(height-height*0.98)+horizonAngle);
  vertex(width-width*0.88, horizonAngle);
  vertex(width-width*0.908, horizonAngle);
  endShape();
  ...
  rectMode(CENTER);
  fill(255, 0, 0);
  rect(width-width*0.833, horizonAngle, width-width*0.925, height-height*0.962);
  rectMode(CENTER);
  fill(255, 255, 255);
  rect(width-width*0.833, horizonAngle, width-width*0.95, height-height*0.975);
  textFont(charFont);
  stroke(0);
  fill(0);
  textSize(15);
  text(degree+"°", width-width*0.845, horizonAngle+5);
  popMatrix();
}
...
void basePart(int an) {
  
  int degree = int(map(an,-125,125,-90,90));
  fill(#5A25F2);
  ...
  fill(#FF74DD);
  textFont(numFont, 20);
  text(degree, width-width*0.85, height-height*0.15);
  stroke(0);
  strokeWeight(4);
  noFill();
